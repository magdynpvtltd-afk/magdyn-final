    ...
    <div class="form-page">
        ...
        <div class="form-page-body" style="max-width:740px;">
            ...
            <div style="display:flex;gap:10px;align-items:center;margin-bottom:14px;">
                <button type="button" class="btn btn-primary" id="tpl-import-run-btn"
            ...
            <script>
            (function () {
                var btn = document.getElementById('tpl-import-run-btn');
                if (!btn) return;
                ...
                btn.addEventListener('click', async function () {
                    if (!confirm('Import all inspection templates from the old system?\n\nExisting imported templates (same pid) will be rebuilt.')) return;
                    btn.disabled = true;
                    ...
                    try {
                        while (!done) {
                            var d = await post({ action: 'import_old_templates_chunk', offset: offset });
                        ...
                        btn.textContent = '▶ Import Templates';
                        btn.disabled = false;
                    }
                });
            })();
    ...
    $rowRenderer = function ($r) use ($canCreate, $canManage) {
        ...
        return [
            ...
            'target_count'    => (int)$r['target_count']
                                  ? (int)$r['target_count']
                                  : '<span class="muted small">any</span>',
